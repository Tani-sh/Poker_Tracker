          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                subtitle: player.isLinked
                    ? Text('📱 ${player.phoneNumber}', style: TextStyle(fontSize: 11, color: Colors.grey.shade500))
                    : null,
                ...
                trailing: _showArchived
                    ...
                    : Row(
                        ...
                        children: [
                          if (!player.isLinked)
                            IconButton(
                              icon: const Icon(Icons.phone_android, color: Colors.tealAccent, size: 20),
                                ...
                                showModalBottomSheet(
                                  ...
                                  builder: (_) => ContactPickerSheet(
                                    onContactSelected: (name, phone) {
                                      final dup = Hive.box<Player>('players').values
                                          .where((p) => p.phoneNumber == phone && p.key != player.key)
                                          .toList();
                                      if (dup.isNotEmpty) {
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(content: Text('⚠️ ${dup.first.name} already has this number')),
                                        );
                                        return;
                                      }
                            ...
                            IconButton(
                              ...
                              onPressed: () {
                                player.unlinkPhone();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text('🔗 ${player.name} unlinked')),
                                );
                              },
                            ),
                          IconButton(
