...
import 'link_players_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // Auto-sync in the background on app launch
          ...
          return Padding(
            ...
            child: Column(
              children: [
                ...
                const Text('No games yet!', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                Text('Start your first cash game to see the leaderboard here.',
        ...
        actions: [
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    ...
                    messenger.showSnackBar(
                      const SnackBar(content: Text('☁️ Syncing...'), duration: Duration(seconds: 1)));
                    final result = await FirestoreService().fullSync();
                    if (mounted) setState(() {});
                    messenger.showSnackBar(
                      SnackBar(content: Text('✅ Uploaded ${result['uploaded']}, downloaded ${result['downloaded']} games & ${result['payments']} payments')));
                  },
                ),
                if (FirestoreService().hasPendingSyncs)
                  Positioned(
                    right: 6, top: 6,
                    child: Container(
                      width: 10, height: 10,
          ...
          PopupMenuButton<String>(
            icon: Icon(
              ...
              color: AuthService().isLoggedIn ? Colors.tealAccent : null,
            ),
            tooltip: 'Account',
            onSelected: (value) async {
              switch (value) {
                case 'link_players':
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const LinkPlayersScreen()));
                  break;
                case 'update_app':
                  final url = Uri.parse('https://github.com/Tani-sh/Poker_Tracker/raw/main/PokerTracker.apk');
            ...
            itemBuilder: (_) => [
              if (AuthService().isLoggedIn) ...[
                PopupMenuItem(
                  enabled: false,
                  child: Text('📱 ${AuthService().phoneNumber ?? 'Signed In'}',
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                ),
                const PopupMenuItem(value: 'link_players',
                  child: ListTile(leading: Icon(Icons.people), title: Text('Link Players'), dense: true)),
                const PopupMenuItem(value: 'update_app',
                  child: ListTile(leading: Icon(Icons.system_update), title: Text('Update App'), dense: true)),
                const PopupMenuItem(value: 'sign_out',
                  child: ListTile(leading: Icon(Icons.logout, color: Colors.red), title: Text('Sign Out', style: TextStyle(color: Colors.red)), dense: true)),
              ] else
                const PopupMenuItem(value: 'sign_in',
                  child: ListTile(leading: Icon(Icons.login), title: Text('Sign In'), dense: true)),
            ],
          ),
