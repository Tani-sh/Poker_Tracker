      ...
      builder: (_) => ContactPickerSheet(
        onContactSelected: (name, phone) {
          ...
          if (existing.isNotEmpty) {
            // Auto-select existing player
            final player = existing.first;
            if (!_selectedPlayers.any((p) => p.key == player.key)) {
              setState(() => _selectedPlayers.add(player));
            }
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('✅ ${player.name} already exists — selected!')),
            );
          } else {
            // Create new player with phone
            final newPlayer = Player(name: name, phoneNumber: phone);
            _playersBox.add(newPlayer);
            setState(() => _selectedPlayers.add(newPlayer));
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('✅ $name added with phone $phone')),
            );
          }
        },
      ),
